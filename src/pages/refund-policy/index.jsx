...
export default function Terms({staticPage, defaultImage}) {
    return (
        <main>
            <Head>
                <title>{staticPage.content.refund_seo_title}</title>
                <meta
                    name="description"
                    content={staticPage.content.refund_seo_descriptions}
                />
                <meta
                    name="keyword"
                    content={staticPage.content.refund_seo_keyword}
                />
                <meta
                    property="og:title"
                    content={staticPage.content.refund_seo_title}
                />
                <meta
            ...
            <div className="py-32">
                <div className="container">
                    <SectionHeading style={{textAlign: "center"}}>
                        {staticPage.content.refund_title}
                    </SectionHeading>

                    <div className="text-left text-[0.875rem] lg:text-[1.25rem] pb-20 details">
                        {parse(staticPage.content.refund_content)}
                    </div>
                </div>
            </div>
        </main>
...
export async function getServerSideProps() {
    const staticPage = await fetch(
        `https://cms.pmgdeals.com/api/public/content?page=refund`
    ).then((res) => res.json());
