export default function CheckBox({ id, label, icon, value }) {
  return (
    <label
      key={`label-${id}`}
      htmlFor={`input-${id}`}
      className={`group shrink-0 flex items-center gap-2 rounded-full px-8 py-2 cursor-pointer transition-all duration-300 has-[:checked]:bg-gray-dark hover:bg-gray-dark border-[0.0625rem] border-gray-600`}
    >
